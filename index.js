...
const stepThroughCell = (row, column) => {
	...
	for (const neighbor of neighbors) {
		...
		// See it that neighbor is out of bounds
...
horizontals.forEach((row, rowIndex) => {
	row.forEach((open, columnIndex) => {
		...
		const wall = Bodies.rectangle(x, y, unitLengthX, 5, { label: "wall", isStatic: true, render: { fillStyle: "red" } });
...
verticals.forEach((row, rowIndex) => {
	row.forEach((open, columnIndex) => {
		...
		const wall = Bodies.rectangle(x, y, 5, unitLengthY, { label: "wall", isStatic: true, render: { fillStyle: "red" } });
		World.add(world, wall);
	});
});
